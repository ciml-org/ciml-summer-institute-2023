...
#
# 1
# Review the code below and fill in the blanks from the documentation, or from the slides
#
# Look for the #<<<<<<<<-----------  arrows to mark where the blanks are
#
# Note, keep in mind this exercise uses numpy data matrix that is split up, which is simpler than the example in the documentation.
#
# Finally, you can find a lot more detail on the Horovod api here (which we don't need for this exercise)
# https://horovod.readthedocs.io/en/latest/api.html#module-horovod.tensorflow.keras
#
#
# 3
# In a terminal window, Open the run-hvd-main-cpu2.sb  slurm batch script
#
# execute  ]$  sbatch run-hvd-main-cpu2.sb
...
def build_model():
    ...
    optimizer2use  = hvd.DistributedOptimizer(______)    # <<<<<<--------- set up this wrapper
...
#becareful, early stopping could cause some ranks to finish before others
...
fit_history=multi_worker_model.fit(train_dataset[0],train_dataset[1], 
                   ...
                   callbacks=[_____]    #<<<<<<<<<-------- which call back goes here
                   )
